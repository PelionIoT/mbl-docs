...
| `--distro` | build.sh | Choose which distribution to build. Supported values are: **`mbl-development`** (default) or **`mbl-production`** |
| `--image` | build.sh | Choose which image to build. Supported values are: **`mbl-image-development`** (default when no `--distro` is passed or `--distro mbl-development` ) or **`mbl-image-production`** (default when `--distro mbl-production` ) |

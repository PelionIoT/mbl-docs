...
| `--repo-host` | build.sh | Specify the hostname of an external source code repository that needs to be accessed via SSH in the build environment. This option can be specified multiple times to add more than one host. <br>Example: `--repo-host gitlab.com --repo-host ssh.dev.azure.com` <br>For more information, [see Troubleshooting](../develop-mbl/troubleshooting.html)|

...
1. Check that ECM is working by verifying a valid IP address on the interface and by pinging a website, for example www.google.com:
    ...
    <span class="notes">There is a known issue with DNS lookup using the cellular module, which means you currently have to use an IP address (such as 216.58.213.100), instead of using a website domain name lookup. If you have access to our repositories on GitHub, you can [view this issue and follow its progress](https://github.com/ARMmbed/meta-mbl/issues/468).</span>

...
| Name              | ro/rw | File system type | Mount point      | Default size | Contains |
|-------------------|-------|------------------|------------------|--------------|----------|
| Bank/Update state | -     | -                | -                | 128MiB       | Unused at this time |
| Boot (Bank 1)     | ro    | vfat             | /boot            | 128MiB       | Kernel, device tree and U-Boot boot script |
| Boot (Bank 2)     | -     | vfat             | -                | 128MiB       | Unused at this time |
| Rootfs (Bank 1)   | rw    | ext4             | /                | 512MiB       | Root file system bank 1 |
| Rootfs (Bank 2)   | rw    | ext4             | /                | 512MiB       | Root file system bank 2 |
| Factory config    | rw    | ext4             | /config/factory  | 32MiB        | Factory configuration |
| Config (Bank 1)   | rw    | ext4             | /config/user     | 32MiB        | Configuration bank 1; user configuration data |
| Config (Bank 2)   | -     | ext4             | -                | 32MiB        | Unused at this time |
...
| Name               | Number of banks | Partition type | File system type | Notes |
|--------------------|-----------------|----------------|------------------|-------|
...
| Bootloader FS Slot | 1               | Primary        | vfat             | Only used on boards where the boot ROM loads code from a file system rather than raw storage (e.g. Raspberry Pi 3). |
...
-->
| Number | Name                       | Offset  | Size   | Partition type | File system type | Notes |
| ------ | ----                       | ------  | ----   | -------------- | ---------------- | ----- |
...
| 1      | Bootloader FS slot         | 192MiB  | 48MiB  | Primary        | vfat             | Contains bootloader component 1 and the kernel |
| 2      | Boot (Bank 1)              | 240MiB  | 128MiB | Primary        | vfat             | Unused |
| 3      | Boot (Bank 2)              | 368MiB  | 128MiB | Primary        | vfat             | Unused |
| 4      | -                          | -       | -      | Extended       | -                | - |
| 5      | Rootfs (Bank 1)            | 512MiB  | 512MiB | Logical        | ext4             | - |
| 6      | Rootfs (Bank 2)            | 1040MiB | 512MiB | Logical        | ext4             | - |
| 7      | Factory config             | 1568MiB | 32MiB  | Logical        | ext4             | - |
| 8      | Config (Bank 1)            | 1616MiB | 32MiB  | Logical        | ext4             | - |
| 9      | Config (Bank 2)            | 1664MiB | 32MiB  | Logical        | ext4             | Unused |
...
| -      | Bank/Update state          | 64MiB     | 128MiB | Raw            | -                | Unused |
| 1      | Boot (Bank 1)              | 192MiB    | 128MiB | Primary        | vfat             | Contains the kernel |
| 2      | Boot (Bank 2)              | 320MiB    | 128MiB | Primary        | vfat             | Unused |
| 3      | Rootfs (Bank 1)            | 448MiB    | 512MiB | Primary        | ext4             | - |
| 4      | -                          | -         | -      | Extended       | -                | - |
| 5      | Rootfs (Bank 2)            | 960.5MiB  | 512MiB | Logical        | ext4             | - |
| 6      | Factory config             | 1473MiB   | 32MiB  | Logical        | ext4             | - |
| 7      | Config (Bank 1)            | 1505.5MiB | 32MiB  | Logical        | ext4             | - |
| 8      | Config (Bank 2)            | 1538MiB   | 32MiB  | Logical        | ext4             | Unused |
...
| 3      | Rootfs (Bank 1)            | 448MiB  | 512MiB | Primary        | ext4             | - |
| 4      | -                          | -       | -      | Extended       | -                | - |
| 5      | Rootfs (Bank 2)            | 976MiB  | 512MiB | Logical        | ext4             | - |
| 6      | Factory config             | 1504MiB | 32MiB  | Logical        | ext4             | - |
| 7      | Config (Bank 1)            | 1552MiB | 32MiB  | Logical        | ext4             | - |
| 8      | Config (Bank 2)            | 1600MiB | 32MiB  | Logical        | ext4             | Unused |
...
| `MBL_WKS_STORAGE_DEVICE_NAME`          | None          | The name of the device's main storage device according to Linux, e.g. `"mmcblk1"` (see note 6). |
...
1. By default, the offsets of the second bootloader slot's banks are calculated based on the size of the first bootloader slot and alignment
   ...
   variables.
1. `MBL_WKS_STORAGE_DEVICE_NAME` is the name that the storage's device file has
   in the `/dev` directory and does *not* include a partition number. E.g.
   `mmcblk1`.
...
A script inside the `initramfs` checks which root partition bank is currently active; by default, this is the first bank, but the script changes to using the second bank if a flag file called `rootfs2` is present in the `bootflags` directory of the `log` partition. In the future, state about banks and firmware updates will be moved to the Bank/Update state partition.
